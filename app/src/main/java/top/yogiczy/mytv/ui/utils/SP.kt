...
object SP {
    ...
    enum class KEY {
        ...
        /** 播放器 UA 历史列表 */
        VIDEO_PLAYER_USER_AGENT_HISTORY_LIST,
    ...
    var iptvSourceNameMap: Map<String, String>
        get() {
            val jsonStr = sp.getString(KEY.IPTV_SOURCE_NAME_MAP.name, "{}") ?: "{}"
            return try {
                // 直接手动解析 JSON
        ...
        set(value) {
            ...
            sp.edit().putString(KEY.IPTV_SOURCE_NAME_MAP.name, json.toString()).apply()
        }

    /** 直播源默认源 */
    var iptvSourceDefaultUrl: String
        get() = (sp.getString(KEY.IPTV_SOURCE_DEFAULT_URL.name, "") ?: "").ifBlank {
            Constants.IPTV_SOURCE_URL
        }
        set(value) = sp.edit().putString(KEY.IPTV_SOURCE_DEFAULT_URL.name, value).apply()
    ...
    /**
     * 获取智能降级的 User-Agent
     * 优先级：1. 直播源中的 UA  2. APP 设置的 UA  3. 默认 UA
     */
    fun getSmartUserAgent(extractedUA: String?): String {
        return when {
            !extractedUA.isNullOrBlank() -> {
                Logger.create("SP").i("使用直播源中的 User-Agent: $extractedUA")
                extractedUA
            }
            videoPlayerUserAgent.isNotBlank() && videoPlayerUserAgent != Constants.VIDEO_PLAYER_USER_AGENT -> {
                Logger.create("SP").i("使用 APP 设置的 User-Agent: $videoPlayerUserAgent")
                videoPlayerUserAgent
            }
            else -> {
                Logger.create("SP").i("使用默认 User-Agent: ${Constants.VIDEO_PLAYER_USER_AGENT}")
                Constants.VIDEO_PLAYER_USER_AGENT
            }
        }
    }
    ...
    /** 播放器 UA 历史列表 */
    var videoPlayerUserAgentHistoryList: Set<String>
        get() = sp.getStringSet(KEY.VIDEO_PLAYER_USER_AGENT_HISTORY_LIST.name, emptySet()) ?: emptySet()
        set(value) = sp.edit().putStringSet(KEY.VIDEO_PLAYER_USER_AGENT_HISTORY_LIST.name, value).apply()
